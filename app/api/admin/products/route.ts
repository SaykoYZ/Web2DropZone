...
export async function POST(req: Request) {
  ...
  const p = await new JsonProductRepository().create({
  ...
  await addLog({
    ...
    userId: u.id,
    description: `Produit créé : ${p.name}`,
    createdAt: now,
  });
...
export async function PATCH(req: Request) {
  ...
  const p = await new JsonProductRepository().update(
    b.id,
    data
  );
  ...
  await addLog({
    ...
    userId: u.id,
    description: `Produit modifié : ${p.name}`,
...
export async function DELETE(req: Request) {
  ...
  const ok = await new JsonProductRepository().delete(id);
